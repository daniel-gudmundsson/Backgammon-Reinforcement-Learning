...
def play_a_game(commentary=False, net=None, train=False):
    board = init_board()  # initialize the board
    player = np.random.randint(2) * 2 - 1  # which player begins?
    # play on
    net.search.theta = np.zeros(198)
    net.actor.z = np.zeros(198)
    net.actor.a = None
    ...
    while not game_over(board) and not check_for_error(board):
        ...
        for i in range(1 + int(dice[0] == dice[1])):
            ...
            #        net.actor.a, net.actor.s_prime = net.actor.nextMove(board_copy, dice, player, net.search.theta)
            #        new_move = net.actor.a
                    #old_state = net.actor.s_prime
                #else:
                move, new_state = net.actor.nextMove(board_copy, dice, player, net.search.theta)
            ...
            if len(move) != 0:
                ...
                if(player == 1 and count > 1):
                    ...
                    if(not game_over(new_state) and not check_for_error(new_state)):
                        if(train):
                            net.search.do(old_state, dice, net.actor.theta, player)
                            delta = 0 + net.actor.getValue(new_state, np.zeros(198), player) - net.actor.getValue(old_state, np.zeros(198), player)
                            net.actor.theta = net.actor.theta + net.actor.alpha * delta * net.actor.z
                            net.actor.z = net.actor.lamb * net.actor.z + agent.getFeatures(old_state, player)
                        old_state = new_state  
    ...
    # return the winner
    if(train is True):
        delta = player * -1 + 0 - net.actor.getValue(old_state, np.zeros(198),player * -1)
        net.actor.theta = net.actor.theta + net.actor.alpha * delta * net.actor.z
        net.actor.z = net.actor.lamb * net.actor.z + agent.getFeatures(old_state, player * -1)
